...
def download_minist_dataset(outpath: str, tf: transforms):
    ...
    mnist = datasets.MNIST(outpath, train=True, download=True, transform=tf)
    mnist_test = datasets.MNIST(outpath, train=False, download=True, transform=tf)
...
def download_cyfar10_dataset(outpath: str):  
    ...
    cyfar10 = datasets.CIFAR10(outpath, train=True, download=True, transform=tf)
    # 標準化・正規化の計算    
    loader = DataLoader(cyfar10, batch_size=100, num_workers=2)
    ...
    cyfar10 = datasets.CIFAR10(outpath, train=True, download=True, transform=tf)
    cyfar10_test = datasets.CIFAR10(outpath, train=False, download=True, transform=tf)    
    imgs = []
    loader = DataLoader(cyfar10, batch_size=10, num_workers=2)
    head, _ = next(iter(loader))
    for img in head: 
        img = wandb.Image(img)
        imgs.append(img)
    ...
    loader = DataLoader(cyfar10_test, batch_size=10, num_workers=2)
    head, _ = next(iter(loader))
    imgs = []
    for img in head:
        img = wandb.Image(img)
